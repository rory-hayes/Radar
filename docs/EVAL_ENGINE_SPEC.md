...
RAD-079 links finding-triggered reruns to the evaluation job metadata. When a linked rerun completes with a clean passing result, the job orchestration layer processes the finding link after persisting the run. The default workspace behavior is resolution suggestion: move the finding to Fixed and record activity. An `auto_resolve` metadata mode can complete resolution by writing the required resolution fields and moving the finding to Resolved.
