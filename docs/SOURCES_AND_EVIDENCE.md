...
RAD-054 uses existing source records for Knowledge Runner target configuration instead of adding a separate integration surface. A support bot endpoint or generic API endpoint can be linked to a Knowledge assertion as the customer-facing answer target, while an uploaded document or manual text source can act as an uploaded answer set. Target readiness is derived from source type, endpoint URL, sync status, and content hash. Source config may store HTTP method and credential mode labels, but it must not store raw API keys, bearer tokens, or other runner secrets.
